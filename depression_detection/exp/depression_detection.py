...
DEPRESSION_TRAIN = '../data/training_data.pkl'
DEPRESSION_TEST = '../data/testing_data.pkl'
...
train.to_pickle(DEPRESSION_TRAIN)
test.to_pickle(DEPRESSION_TEST)

# feat = pd.read_excel('data/feature_table.xlsx')
# feat_ff = feat.ffill()
...
training_set = pd.read_pickle(DEPRESSION_TRAIN)
test_set = pd.read_pickle(DEPRESSION_TEST)
...
features_train = tf.constant(training_set.iloc[:,:112].values)
features_test = tf.constant(test_set.iloc[:,:112].values)
labels_train = tf.constant(training_set.iloc[:,112].values)
labels_test = tf.constant(test_set.iloc[:,112].values)
...
result = classifier.evaluate(input_fn=get_test_inputs, steps=2, metrics=metrics)
